...
public class IntegrityCheckinAction extends Notifier implements Serializable
{
  ...
  @Extension
  public static final IntegrityCheckinDescriptorImpl CHECKIN_DESCRIPTOR =
      new IntegrityCheckinDescriptorImpl();

  @DataBoundConstructor
  public IntegrityCheckinAction(String ciConfigPath, String ciWorkspaceDir, String includes,
      String excludes, String serverConfig, String configurationName)
  {
      ...
      throws InterruptedException, IOException
  {
    ...
    {
      LOGGER.fine("Build failed!  Skipping PTC RV&S Checkin step!");
      listener.getLogger().println("Build failed!  Skipping PTC RV&S Checkin step!");
      return true;
    }
	...
	try {
		...
		LOGGER.fine("Exception Caught!  Skipping PTC RV&S Checkin step! :"+ e.getMessage());
	    listener.getLogger().println("Exception Caught!  Skipping PTC RV&S Checkin step!"+ e.getMessage());
	    return true;
	}
