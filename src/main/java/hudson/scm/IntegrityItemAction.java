...
public class IntegrityItemAction extends Notifier implements Serializable
{
  ...
  @Extension
  public static final IntegrityItemDescriptorImpl ITEM_DESCRIPTOR =
      new IntegrityItemDescriptorImpl();
   ...
   * @throws APIException
   */
  private boolean editBuildItem(AbstractBuild<?, ?> build, BuildListener listener,
      String buildItemID) throws IOException, APIException
  {
    IAPICommand editIssueCmd =
        CommandFactory.createCommand(IAPICommand.EDIT_ISSUE_COMMAND, getProjectSettings(build));
    ...
    if (Result.SUCCESS.equals(build.getResult()))
    {
      ...
      listener.getLogger().println("Preparing to update item '" + buildItemID + "' with value "
          + stateField + " = " + successValue);
      mvState.add(successValue);
    } else
    {
      // Failed build update
      listener.getLogger().println("Preparing to update item '" + buildItemID + "' with values "
          + stateField + " = " + failureValue);
   ...
   * @throws APIException
   */
  private void editTestResult(AbstractBuild<?, ?> build, String annotation, String verdict,
      String testSessionID, String testCaseID) throws APIException
  {
    // Setup the edit test result command
    IAPICommand editresultCmd =
        CommandFactory.createCommand(IAPICommand.EDIT_RESULT_COMMAND, getProjectSettings(build));
    ...
    } catch (APIException aex)
    {
      ...
      if (message.indexOf("MKS124746") > 0
          || (message.indexOf("result for test case") > 0 && message.indexOf("does not exist") > 0))
      {
        LOGGER.fine(eh.getCommand() + " returned exit code " + eh.getExitCode());
        LOGGER.fine(eh.getMessage());
        LOGGER.warning(
            "An attempt was made to update a Test Result for non-meaningful content.  Perhaps you've incorrectly configured your Tests?");
      } else
      {
        throw aex;
      }
  ...
  private String getJUnitID(String testCaseID)
  {
    ...
    } else
    {
      ...
      else
      {
        ...
        LOGGER.warning(
            "Invalid format for Test Case ID - should be in the format <packagename>.<classname>.<testname>!");
  ...
  private void updateTestResult(AbstractBuild<?, ?> build, TestResult testResult,
      BuildListener listener, String testSessionID, Response walkResponse, List<Item> testCaseList)
          throws APIException
  {
    ...
    for (Item test : testCaseList)
    {
      ...
      if (null != testCaseIDFld && null != testCaseIDFld.getValueAsString())
      {
        ...
        LOGGER.fine(
            "Looking for external test " + testCaseID + " internal JUnit ID " + junitTestName);
        ...
        if (null != caseResult)
        {
          ...
          editTestResult(build,
              (caseResult.isPassed() ? "Test " + caseResult.getId() + " has passed"
                  : caseResult.getErrorDetails()),
              (caseResult.isPassed() ? testPassedVerdictName : testFailedVerdictName),
              testSessionID, test.getId());
        } else
        {
          // Lets mark the Test Result as skipped for this Test Case
          LOGGER.warning("Could not locate internal JUnit Test - " + junitTestName);
          editTestResult(build, "Test " + getJUnitID(testCaseID) + " was not run!",
              testSkippedVerdictName, testSessionID, test.getId());
        }
      }
      ...
      {
        updateTestResult(build, testResult, listener, testSessionID, walkResponse,
            containsFld.getList());
      }
    }
  }
  ...
  private TestResult getTestResult(AggregatedTestResultAction testResultAction)
  {
    ...
    for (ChildReport childReport : cList)
    {
      if (childReport.result instanceof TestResult)
      {
        ...
        Iterator<? extends TestResult> failedResultIterator =
            testResult.getFailedTests().iterator();
   ...
   */
  @SuppressWarnings("unchecked")
  private boolean collectTestResults(AbstractBuild<?, ?> build, BuildListener listener,
      String testSessionID) throws APIException, AbortException
  {
    // Read the Test Case relationships from the Test Session
    IAPICommand walk =
        CommandFactory.createCommand(IAPICommand.RELATIONSHIPS_COMMAND, getProjectSettings(build));
    ...
    if (null != walkResponse)
    {
      Field testSessionFld =
          walkResponse.getWorkItem(testSessionID).getField(testSessionTestsField);
      if (null != testSessionFld && null != testSessionFld.getList())
      {
        updateTestResult(build, getTestResult(build.getAction(AggregatedTestResultAction.class)),
            listener, testSessionID, walkResponse, testSessionFld.getList());
      }
    }
    ...
    try
    {
      ...
      if (intBuildItemID <= 0)
      {
        if (queryDefinition.length() > 0)
        {
          ...
          IAPICommand issueCmd =
              CommandFactory.createCommand(IAPICommand.ISSUES_COMMAND, getProjectSettings(build));
      ...
      if (null != testResult && testResult.getTotalCount() > 0)
      {
        ...
        if (intTestSessionID <= 0 && testSessionField.length() > 0 && intBuildItemID > 0)
        {
          ...
          IAPICommand walk = CommandFactory.createCommand(IAPICommand.RELATIONSHIPS_COMMAND,
              getProjectSettings(build));
            ...
            if (null != testSessionFld && null != testSessionFld.getList())
            {
              ...
              for (Item session : sessionList)
              {
                ...
                } catch (NoSuchElementException nsee)
                {
                  // 10.6 and beyond...
                  stateField =
                      walkResponse.getWorkItem(session.getId()).getField(testSessionStateField);
                }

                if (null != stateField
                    && testSessionActiveState.equals(stateField.getValueAsString()))
                {
        ...
        // Update the Test Session with the results from the JUnit tests, if we got a Test Session
        // to work with...
   ...
   * object. This means the descriptor is used to create instances of the describable. Usually the
   * Descriptor is an internal class in the IntegrityItemAction class named DescriptorImpl.
   */
  public static class IntegrityItemDescriptorImpl extends BuildStepDescriptor<Publisher>
  {
    public static final String defaultQueryDefinition =
        "((field[Type] = \"Build Request\") and (field[State] = \"Approved\"))";
